...
lollipopPlot = function(maf, gene = NULL, AACol = NULL, labelPos = NULL, labPosSize = 0.9, showMutationRate = TRUE,
  ...
  gff<-gff[!is.na(gff$pfam),]
  #gff$Label <- gff$Description

  #gff = read.table("~/Downloads//protein_domain.hsapiens_gene_ensembl.txt",header = T,sep = "\t")
  # load(file = "~/Downloads/pfamDomain.rda")
  # gff = ensembl_prot_info
  # gff$HGNC <- gff$hgnc_symbol
  # gff$refseq.ID <- gff$refseq_mrna
  # gff$protein.ID <- gff$ensembl_peptide_id
  # gff$aa.length <- gff$peptide_length
  # gff$Start <- gff$start_position
  # gff$End <- gff$end_position
  # data.table::setDT(x = gff)
  ...
  #prot = gff[hgnc_symbol %in% geneID]
  # library(PFAM.db)
  # prot<-prot[prot$pfam != "",]
  # pfam_descriptions <- as.list(PFAMDE[mappedkeys(PFAMDE)])
  # prot$Label <- unlist(pfam_descriptions[match(prot$pfam, names(pfam_descriptions))])

  if(nrow(prot) == 0){
    stop(paste('Structure for protein', geneID, 'not found.', sep=' '))
  }

  # domain_data_src = system.file('extdata', 'protein_domains.RDs', package = 'maftools')
  # domain_data = readRDS(file = domain_data_src)
  #
  # # your  SLAIN1 protetin structure (refseq.ID column is not necessary)
  # slain_motif = data.table::data.table(
  #   HGNC = "SLAIN1",
  #   aa.length = 568,
  #   Start = 187,
  #   End = 568,
  #   Label = "SLAIN",
  #   refseq.ID = "NM_001242868"
  # )
  #
  # print(slain_motif)
  # HGNC aa.length Start End Label    refseq.ID
  # 1: SLAIN1       568   187 568 SLAIN NM_001242868
  #
  # # Add the protein structure to database
  # domain_data = data.table::rbindlist(
  #   l = list(domain_data, slain_motif),
  #   use.names = TRUE,
  #   fill = TRUE
  # )
  #
  # # save back the database
  # saveRDS(object = domain_data, file = domain_data_src)
  ...
  ###Updated for Taking care of the overlapping domains and domains labels
  if(geneID == "B2M")
  {
    prot$Label<-"C1-set"
  }else if(geneID == "XIAP")
  {
    pXIAP<-prot[grepl("zf-C3HC4_3",prot$Label),]
    pNot<-prot[!grepl("zf-C3HC4",prot$Label),]
    prot<-rbind(pNot,pXIAP[1,])
    prot[grepl("zf-C3HC4_3",prot$Label),]$Label<-"zf-C3HC4\n3"
  }else if(geneID == "CARD11")
  {
    prot[grepl("CARD_CARD11_CARMA1",prot$Label),]$Label<-"CARD"
    prot[grepl("PDZ_signaling",prot$Label),]$Label<-"PDZ\nsignaling"
    prot[grepl("P-loop_NTPase",prot$Label),]$Label<-"P-loop\nNTPase"
  }
  else if(geneID == "PTPRD")
  {
    pPTCNot<-prot[!grepl("PTPc",prot$Label),]
    pPTC<-rbind(pPTCNot,prot[grepl("PTPc",prot$Label) & prot$Start == 1382,])
    pPTC<-rbind(pPTC,prot[grepl("PTPc",prot$Label) & prot$Start == 1643,])
    pNotIG<-pPTC[!grepl("I-set|Ig3_RPTP_IIa_LAR_like|Ig2_RPTP_IIa_LAR_like|IG_like|IG",pPTC$Label),]
    pFinal<-rbind(pNotIG,pPTC[grepl("I-set",pPTC$Label) & pPTC$Start == 24,][1,],pPTC[grepl("I-set",pPTC$Label) & pPTC$Start == 238,],pPTC[grepl("Ig2_RPTP_IIa_LAR_like",pPTC$Label) & pPTC$Start == 143,])
    pFinal[grepl("Ig2_RPTP_IIa_LAR_like",pFinal$Label),]$Label<-"I-set"
    prot<-pFinal
  }
